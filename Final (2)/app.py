...
def listings():
    ...
    radius = request.args.get('radius', type=int, default=50)  # Default 50km radius
    ...
    # Filter by distance if user is logged in and has location
...
def view_cart():
    ...
    total_price = sum(item.listing.price * item.quantity for item in cart_items if item.listing.price)
...
def new_trade(listing_id):
    ...
    if request.method == "POST":
        ...
        # Validate trade type
        if trade_type not in ["purchase", "exchange", "loan"]:
...
def update_trade_status(trade_id):
    ...
    # Check if user is the receiver (only receiver can update status)
    ...
    # If accepted and it's a purchase, mark listing as inactive
    if new_status == "accepted" and trade.trade_type == "purchase":
        listing = Listing.query.get(trade.listing_id)
        listing.is_active = False
        db.session.commit()
    
    # Create notification for initiator
    status_message = "accepted" if new_status == "accepted" else "rejected" if new_status == "rejected" else "marked as completed"
    create_notification(
        trade.initiator_id,
    ...
    flash(f"Trade {status_message} successfully!", "success")
    return redirect(url_for("view_trade", trade_id=trade.id))
...
def place_order():
    ...
    # For now, we'll just create trades for each item
    
    for item in cart_items:
        # Create a new trade for each item
        trade = Trade(
            initiator_id=session['user_id'],
        ...
        create_notification(
            ...
            f"You have received a new purchase request for your listing: {item.listing.title}",
            "trade",
            trade.id
        )
